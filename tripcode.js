    ...
    String.prototype.tripcode = function (appendRawKey) {
        ...
        if (key.length >= 12) {
            ...
            switch (key[0]) {
                ...
                // 15桁トリップに変換 (scのみ)
                case "$":
                    tripcode = btoa(sha1.bin(key))
                        .substr(3, 15)
                        .replace("+", ".")
                        .replace("/", "!");

                    // キー2文字目が半角カナの場合はトリップを半角カナに置換
                    if (key.match(halfWidthKanaPattern)) {
                        tripcode = [...tripcode].map(toHalfWidthKana).join("");
                        tripcode = escape(tripcode);
                        tripcode = UnescapeSJIS(tripcode);
                    }
                    break;
